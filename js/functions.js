...
function start(){
  ...
  canvas.addEventListener('click'    , canvas_mouseclick) ;
...
function create_game(){
  xmlhttp_coord = GetXmlHttpObject() ;
  var x = xmlhttp_coord ;
  var request = 'action=create_game' ;
  x.onreadystatechange = parse_created_task ;
  x.open('GET', 'request.php?'+request+'&sid='+Math.random(),true) ;
  x.send(null) ;
}
function parse_created_game(){
  var x = xmlhttp_coord ;
  if(x.readyState==4){
    game_id = parseInt(x.responseText) ;
    var request = 'action=wait_for_rollcall' ;
    x.onreadystatechange = wait_for_rollcall ;
    x.open('GET', 'request.php?'+request+'&sid='+Math.random(),true) ;
    x.send(null) ;
  }
}
...
function create_task(desk_name){
  ...
  x.onreadystatechange = parse_acknowledged_task ;
  x.open('GET', 'request.php?'+request+'&sid='+Math.random(),true) ;
  x.send(null) ;
...
function parse_acknowledged_task(){
  if(nLives<=0){
    endgame() ;
    return ;
  }
  ...
  if(xmlhttp_coordToDesk[desk_name].readyState==4){
    ...
    td.innerHTML = 'acknowledged' ;
    td.style.color = 'black' ;
    td.style.backgroundColor = 'yellow' ;
    
    var request = 'action=harvest_task&task_id='+task_id+'&game_id='+game_id ;
    var x = xmlhttp_coordToDesk[desk_name] ;
    x.onreadystatechange = parse_resolved_task ;
    x.open('GET', 'request.php?'+request+'&sid='+Math.random(),true) ;
    x.send(null) ;
  }
}

function parse_resolved_task(){
  if(nLives<=0){
    endgame() ;
    return ;
  }
