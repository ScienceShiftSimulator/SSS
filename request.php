...
function wait_for_rollcall(){
  global $mysqli ;
  $game_id = $mysqli->escape_string() ;
  $success = false ;
  $counter = 0 ;
  while($success==false){
    $team_rows = gather_team($_GET['game_id']) ;
    $has_coord = false ;
    $has_A     = false ;
    foreach($team_row as $row){
      if($row['role']=='coord') $has_coord = true ;
      if($row['role']=='A'    ) $has_A     = true ;
      if($has_coord && $has_A) $success = true ;
    }
    if($success){
      print_r($team_rows) ;
      break ;
...
function join_game(){
  $role  = $_GET['role'] ;
  $query = 'SELECT * FROM beam_teams WHERE status="open" ORDER BY game_id DESC' ;
  $highest_id = -1 ;
  $result = $mysqli->query($query) ;
  if($result){
...
function create_task(){
  ...
  $game_id = $mysqli->escape_string($_GET['game_id']) ;
  $role    = $mysqli->escape_string($_GET['role'   ]) ;
  $query_read = 'SELECT * FROM beam_bulletin WHERE id=' . $id . ' AND role="' . $role . '" AND status="acknowledged" ORDER BY id ASC' ;
  $response = wait_for_query($query_read, 5, $_GET['game_id']) ;
  $row = $response[0] ;
  echo $row['task_id'] , ':' , $row['outcome'] , ':' , $row['score'] ;
}

function harvest_task(){
  global $mysqli ;
  $fields = [] ;
  $game_id = $mysqli->escape_string($_GET['game_id']) ;
  $task_id = $mysqli->escape_string($_GET['task_id']) ;
  $query_read = 'SELECT * FROM beam_bulletin WHERE task_id=' . $task_id . ' AND game_id="' . $game_id . '" AND status="resolved" ORDER BY id ASC' ;
  $response = wait_for_query($query_read, 10, $_GET['game_id']) ;
  if(count($response)>0){
    $row = $response[0] ;
    echo $row['task_id'] , ':' , $row['outcome'] , ':' , $row['score'] ;
  }
...
if(isset($_GET['action'])){
  ...
  else if($action=='wait_for_rollcall'){
    wait_for_rollcall() ;
  }
